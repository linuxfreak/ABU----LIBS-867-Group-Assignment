...
The Library Index System is designed to manage operations across multiple library branches within Nigeria, with a focus on Nigerian and African literature. The system provides:
...
- **Member management** with differentiated membership tiers
...
- Nigerian membership types (Standard, Student, Senior, Premium, Staff, Librarian)
...
- Centralized acquisition reporting
- Localized member services

#### BR-002: Subject-Based Classification
**Requirement**: Organize books using a hierarchical subject taxonomy supporting African literature, Nigerian curriculum subjects, and international classifications.
...
- Gender field includes "Not Provided" default respecting privacy
...
**Design Notes**:
- ISBN validation supports ISBN-10, ISBN-13, and hyphenated formats
...
#### 8.1.1 Book Availability Search
```sql
-- Optimized with indexes: idx_book_title, idx_copy_availability, idx_copy_book_branch
SELECT b.title, br.name, COUNT(bc.id) AS available
FROM book b
JOIN book_copy bc ON b.id = bc.book_id
JOIN branch br ON bc.branch_id = br.id
WHERE b.title LIKE '%Things Fall Apart%'
  AND bc.availability_status = 'Available'
GROUP BY b.id, br.id;
```

**Optimization**:
- LIKE pattern with leading wildcard prevents index use; consider FULLTEXT for better performance
- Composite index idx_copy_book_branch accelerates JOIN
- Covering index on availability_status filters efficiently

#### 8.1.2 Overdue Loan Identification
```sql
-- Optimized with indexes: idx_loan_overdue (due_date, status)
SELECT m.email, b.title, l.due_date, DATEDIFF(CURDATE(), l.due_date) AS days_overdue
FROM loan l
...
**Optimization**:
- Composite index idx_loan_overdue (due_date, status) enables index-only scan
- JOIN order: loan (smallest filtered set) → member → book_copy → book

#### 8.1.3 Member Borrowing History
```sql
-- Optimized with indexes: idx_member_number, idx_loan_member
SELECT b.title, l.loan_date, l.return_date, l.status
FROM member m
JOIN loan l ON m.id = l.member_id
JOIN book_copy bc ON l.copy_id = bc.id
JOIN book b ON bc.book_id = b.id
WHERE m.membership_number = 'MEM2024001'
ORDER BY l.loan_date DESC;
```

**Optimization**:
- Unique index on membership_number provides O(1) lookup
- idx_loan_member accelerates JOIN
- ORDER BY on loan_date benefits from idx_loan_dates

### 8.2 Index Usage Analysis

#### 8.2.1 Covering Indexes

Some queries can be satisfied entirely from index without table access:

```sql
-- Query can use idx_copy_book_branch_status covering index
SELECT book_id, branch_id, availability_status
FROM book_copy
WHERE book_id = 6 AND branch_id = 1;

-- Potential covering index (not implemented):
CREATE INDEX idx_cover_copy ON book_copy(book_id, branch_id, availability_status);
```

#### 8.2.2 Full-Text Search

```sql
-- Optimized with FULLTEXT index
SELECT id, title, MATCH(title, subtitle, description) AGAINST('Nigerian war biafra' IN NATURAL LANGUAGE MODE) AS relevance
FROM book
WHERE MATCH(title, subtitle, description) AGAINST('Nigerian war biafra' IN NATURAL LANGUAGE MODE)
...
### 8.3 Query Performance Considerations

#### 8.3.1 JOIN Optimization

**Best Practices Implemented**:
1. **Filter early**: WHERE clauses on driving table reduce JOIN set size
2. **Index foreign keys**: All FK columns indexed automatically
3. **Composite indexes**: Multi-column indexes for frequent JOIN + WHERE patterns
4. **Avoid SELECT ***: Specify only needed columns (reduces I/O)

#### 8.3.2 Subquery vs JOIN

```sql
-- SUBQUERY (slower - multiple executions)
SELECT * FROM book
WHERE id IN (SELECT book_id FROM book_copy WHERE availability_status = 'Available');

-- JOIN (faster - single execution with index)
SELECT DISTINCT b.* FROM book b
JOIN book_copy bc ON b.id = bc.book_id
WHERE bc.availability_status = 'Available';
```

**Recommendation**: Use JOINs with DISTINCT over correlated subqueries.
...
**Alternative for large offsets** (better performance):
```sql
-- Keyset pagination (requires last seen ID)
SELECT id, title FROM book
WHERE id > 40
ORDER BY id
LIMIT 20;
...
#### 9.2.1 Search Functionality Tests
...
#### 9.2.3 Integrity Constraint Tests
...
#### 9.3.1 Query Execution Time Targets
...
Refer to Section 9.2 for comprehensive test queries. Key queries include:
